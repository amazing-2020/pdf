...
class Move:
    ...
    def _update_rectangle(self):
        ...
        print(self.stats.bullet_amount, " ", self.stats.number_of_hits)
    ...
    def _check_missed(self):
        if self.stats.bullet_amount - self.stats.number_of_hits >= 3:
            self.space_ship.center_ship()
            self.bullets.empty()
            self.stats.reset_stats()
            sleep(0.5)
    ...
    def _update_screen(self):
        ...
        if self.stats.game_active:
            self.reset_button.rect.midtop = self.screen.get_rect().midtop
            self.reset_button.prep_msg("Reset")
